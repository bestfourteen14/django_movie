...
class CommentDetail(generics.ListCreateAPIView):
    ...
    #     duplicated_names = queryset.values('title_kor').annotate(name_count=Count('title_kor')).filter(name_count__gt=1)
    #     for title in duplicated_names:
    #         duplicates = queryset.filter(title_kor=title['title_kor'])
    #         duplicates.exclude(pk=duplicates.first().pk).delete()
    #     return queryset
